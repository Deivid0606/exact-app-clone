...
export default function Index() {
  ...
  useEffect(() => {
    ...
    if (currentView === 'auth') {
      const defaultView =
        profile.role === 'DESPACHANTE' || profile.role === 'DELIVERY'
          ? 'orders'
          : 'dashboard';
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="flex items-center gap-3">
          <span className="btn-spinner !h-6 !w-6 !border-brand !border-t-brand-glow" />
    ...
    return (
      <div className="mx-auto max-w-full px-6 py-5">
        ...
        <div className="app-card py-12 text-center">
          <h3 className="mb-3 text-xl font-extrabold">Cuenta pendiente de aprobación</h3>
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'assignOrders':
        return <AssignOrdersView />;

      case 'rankingDelivery':
        return <RankingDeliveryView />;

      case 'rendicionesPagadas':
        return <RendicionesPagadasView />;

      case 'withGuides':
        return <WithGuidesView />;
      ...
      case 'shopifyInbox':
        return <ShopifyInboxView onSheetConfirm={handleSheetConfirm} />;
