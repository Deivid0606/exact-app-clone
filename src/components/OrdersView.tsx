...
export default function OrdersView() {
  ...
  const loadOrders = async () => {
    ...
    // Aplicar filtro de proveedor manual si existe (solo para ADMIN/DESPACHANTE)
    if (providerFilter && (role === 'ADMIN' || role === 'DESPACHANTE')) {
      ordersData = ordersData.filter(order => isProviderAllowed(order, providerFilter));
    }
  ...
  return (
    <div className="app-card">
      <h3 className="text-lg font-extrabold mb-3">
        {isGuidesTab ? 'Pedidos con guías' : 'Pedidos'}
      </h3>
